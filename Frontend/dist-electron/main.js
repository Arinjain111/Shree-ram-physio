...
class LocalDatabase {
  db;
  constructor(dbPath) {
    const finalPath = dbPath || path__namespace.join(electron.app.getPath("userData"), "clinic_data.db");
...
class SyncEngine {
  ...
  async checkConnectivity() {
    try {
      await this.apiClient.get("/health", { timeout: 5e3 });
      return true;
    } catch {
      return false;
    }
...
electron.ipcMain.handle("get-sync-status", async () => {
  try {
    ...
    const pendingCount = database.getPendingRecords("patients").length + database.getPendingRecords("invoices").length + database.getPendingRecords("treatments").length;
