...
function log(message) {
  try {
    const logPath = path__namespace.join(electron.app.getPath("userData"), "native_loader.log");
    const timestamp = (/* @__PURE__ */ new Date()).toISOString();
    fs__namespace.appendFileSync(logPath, `${timestamp} ${message}
`);
  } catch (e) {
  }
}
...
function loadBetterSqlite3() {
  patchModuleLoading();
  try {
    const modulePath = electron.app.isPackaged ? path__namespace.join(process.resourcesPath, "better-sqlite3") : "better-sqlite3";
...
function loadPrismaAdapter() {
  patchModuleLoading();
  try {
    const modulePath = electron.app.isPackaged ? path__namespace.join(process.resourcesPath, "@prisma", "adapter-better-sqlite3") : "@prisma/adapter-better-sqlite3";
...
function loadNativeModules() {
  if (Database && PrismaBetterSqlite3) return;
  try {
...
function logToFile(message) {
  try {
    const logPath = path__namespace.join(electron.app.getPath("userData"), "startup_debug.log");
    const timestamp = (/* @__PURE__ */ new Date()).toISOString();
    fs__namespace.appendFileSync(logPath, `${timestamp} [PrismaLoader] ${message}
`);
  } catch (e) {
  }
}
...
function getPrismaClientConstructor() {
  ...
  try {
    if (electron.app.isPackaged) {
      ...
      possiblePaths.push(path__namespace.join(process.resourcesPath, ".prisma", "client", "index.js"));
      possiblePaths.push(path__namespace.join(process.resourcesPath, "node_modules", ".prisma", "client", "index.js"));
      possiblePaths.push(path__namespace.join(process.resourcesPath, "@prisma", "client", "index.js"));
      ...
      const resourcesContent = fs__namespace.existsSync(process.resourcesPath) ? fs__namespace.readdirSync(process.resourcesPath).join(", ") : "Resources folder not found";
...
function getPrismaClient() {
  if (!prismaClient) {
    ...
    try {
      ...
      if (!fs__namespace.existsSync(dbDir)) fs__namespace.mkdirSync(dbDir, { recursive: true });
...
async function initializeDatabase() {
  const Database2 = loadBetterSqlite3();
  const dbPath = path__namespace.join(electron.app.getPath("userData"), "shri-ram-physio.db");
...
const httpAdapter = isHttpAdapterSupported && function httpAdapter2(config) {
  return wrapAsync(async function dispatchHttpRequest(resolve, reject, onDone) {
    ...
    const abortEmitter = new require$$11.EventEmitter();
...
class PrismaSyncEngine {
   ...
   */
  startAutoSync(intervalMs = 5 * 60 * 1e3) {
   ...
   */
  resetAutoSyncTimer(intervalMs = 5 * 60 * 1e3) {
  ...
  async shouldSync() {
    try {
      ...
      const statusResponse = await axios$1.get(`${this.backendUrl}/api/sync/status`, { timeout: 5e3 });
  ...
  async performSync(force = false) {
    ...
    try {
      ...
      const syncPayload = {
        lastSyncTime: null,
        // Always fetch ALL from cloud, not just recent updates
      ...
      try {
        ...
        const presetsResponse = await axios$1.get(`${this.backendUrl}/api/presets`, {
          timeout: 1e4
        });
      ...
      return {
        success: true,
        message: totalDownloaded > 0 ? `Downloaded ${totalDownloaded} records from cloud` : totalUploaded > 0 ? `Uploaded ${totalUploaded} records to cloud` : "All data is up to date",
  ...
  async checkConnectivity() {
    try {
      await axios$1.get(`${this.backendUrl}/health`, { timeout: 5e3 });
      return true;
    } catch {
      return false;
    }
...
function requireAxios() {
  ...
  hasRequiredAxios = 1;
  const FormData$12 = requireForm_data();
  const crypto = require$$3;
  ...
  const FormData__default = /* @__PURE__ */ _interopDefaultLegacy(FormData$12);
  const crypto__default = /* @__PURE__ */ _interopDefaultLegacy(crypto);
  const url__default = /* @__PURE__ */ _interopDefaultLegacy(url);
  const proxyFromEnv__default = /* @__PURE__ */ _interopDefaultLegacy(proxyFromEnv2);
  const http__default = /* @__PURE__ */ _interopDefaultLegacy(http);
  const https__default = /* @__PURE__ */ _interopDefaultLegacy(https);
  const http2__default = /* @__PURE__ */ _interopDefaultLegacy(http2$1);
  ...
  const kindOfTest2 = (type2) => {
    type2 = type2.toLowerCase();
    return (thing) => kindOf2(thing) === type2;
  };
  const typeOfTest2 = (type2) => (thing) => typeof thing === type2;
  ...
  AxiosError2.from = (error, code, config, request, response, customProps) => {
    ...
    const msg = error && error.message ? error.message : "Error";
    const errCode = code == null && error ? error.code : code;
    AxiosError2.call(axiosError, msg, errCode, config, request, response);
    if (error && axiosError.cause == null) {
      Object.defineProperty(axiosError, "cause", { value: error, configurable: true });
    }
    axiosError.name = error && error.name || "Error";
  ...
  const transitionalDefaults2 = {
    silentJSONParsing: true,
    forcedJSONParsing: true,
    clarifyTimeoutError: false
  };
  const URLSearchParams2 = url__default["default"].URLSearchParams;
  ...
  const parseHeaders2 = (rawHeaders) => {
    const parsed = {};
    let key;
    let val;
    let i;
  ...
  const AxiosHeaders$12 = AxiosHeaders3;
  function transformData2(fns, response) {
    const config = this || defaults$1;
    const context = response || config;
    const headers = AxiosHeaders$12.from(context.headers);
    let data = context.data;
  ...
  function CanceledError2(message, config, request) {
    AxiosError2.call(this, message == null ? "canceled" : message, AxiosError2.ERR_CANCELED, config, request);
    this.name = "CanceledError";
  }
  ...
  function fromDataURI2(uri2, asBlob, options) {
    const _Blob = options && options.Blob || platform2.classes.Blob;
    const protocol = parseProtocol2(uri2);
    if (asBlob === void 0 && _Blob) {
      asBlob = true;
    }
    if (protocol === "data") {
      uri2 = protocol.length ? uri2.slice(protocol.length + 1) : uri2;
      const match = DATA_URL_PATTERN2.exec(uri2);
      if (!match) {
        throw new AxiosError2("Invalid URL", AxiosError2.ERR_INVALID_URL);
      }
      const mime = match[1];
      const isBase64 = match[2];
      const body = match[3];
      ...
      if (asBlob) {
        if (!_Blob) {
          throw new AxiosError2("Blob is not supported", AxiosError2.ERR_NOT_SUPPORT);
        }
        return new _Blob([buffer], { type: mime });
      }
      return buffer;
    }
    throw new AxiosError2("Unsupported protocol " + protocol, AxiosError2.ERR_NOT_SUPPORT);
  }
  const kInternals2 = /* @__PURE__ */ Symbol("internals");
  class AxiosTransformStream2 extends stream__default["default"].Transform {
    constructor(options) {
      options = utils$12.toFlatObject(options, {
        maxRate: 0,
        chunkSize: 64 * 1024,
  ...
  const AxiosTransformStream$1 = AxiosTransformStream2;
  const { asyncIterator: asyncIterator2 } = Symbol;
  ...
  const readBlob$1 = readBlob2;
  const BOUNDARY_ALPHABET2 = platform2.ALPHABET.ALPHA_DIGIT + "-_";
  const textEncoder2 = typeof TextEncoder === "function" ? new TextEncoder() : new util__default["default"].TextEncoder();
  const CRLF2 = "\r\n";
  ...
  const formDataToStream$1 = formDataToStream2;
  class ZlibHeaderTransformStream2 extends stream__default["default"].Transform {
    __transform(chunk, encoding, callback) {
      this.push(chunk);
      callback();
    }
  ...
  const ZlibHeaderTransformStream$1 = ZlibHeaderTransformStream2;
  ...
  const callbackify$1 = callbackify2;
  ...
  const isBrotliSupported2 = utils$12.isFunction(zlib__default["default"].createBrotliDecompress);
  const { http: httpFollow2, https: httpsFollow2 } = followRedirects__default["default"];
  ...
  const resolveFamily2 = ({ address, family }) => {
  ...
  const buildAddressEntry2 = (address, family) => resolveFamily2(utils$12.isObject(address) ? address : { address, family });
  ...
  const httpAdapter3 = isHttpAdapterSupported2 && function httpAdapter4(config) {
    return wrapAsync2(async function dispatchHttpRequest(resolve, reject, onDone) {
      ...
      const isHttp2 = httpVersion === 2;
      if (lookup) {
        const _lookup = callbackify$1(lookup, (value) => utils$12.isArray(value) ? value : [value]);
        lookup = (hostname, opt, cb) => {
          _lookup(hostname, opt, (err, arg0, arg1) => {
            if (err) {
              return cb(err);
      ...
      function abort(reason) {
        try {
          abortEmitter.emit("abort", !reason || reason.type ? new CanceledError2(null, config, req) : reason);
        } catch (err) {
          console.warn("emit error", err);
        }
      }
      ...
      if (protocol === "data:") {
        ...
        try {
          ...
          throw AxiosError2.from(err, AxiosError2.ERR_BAD_REQUEST, config);
        }
        if (responseType === "text") {
          convertedData = convertedData.toString(responseEncoding);
          if (!responseEncoding || responseEncoding === "utf8") {
      ...
      const headers = AxiosHeaders$12.from(config.headers).normalize();
      ...
      } else if (utils$12.isFormData(data) && utils$12.isFunction(data.getHeaders)) {
        headers.set(data.getHeaders());
        if (!headers.hasContentLength()) {
          try {
      ...
      req = transport.request(options, function handleResponse(res) {
        ...
        responseStream = streams.length > 1 ? stream__default["default"].pipeline(streams, utils$12.noop) : streams[0];
        const response = {
          status: res.statusCode,
          statusText: res.statusMessage,
          headers: new AxiosHeaders$12(res.headers),
          config,
          request: lastRequest
        };
        if (responseType === "stream") {
          ...
          responseStream.on("end", function handleStreamEnd() {
            try {
              ...
              return reject(AxiosError2.from(err, null, config, response.request, response));
            }
            settle2(resolve, reject, response);
          });
        }
      ...
      if (config.timeout) {
        ...
        req.setTimeout(timeout, function handleRequestTimeout() {
          if (isDone) return;
          let timeoutErrorMessage = config.timeout ? "timeout of " + config.timeout + "ms exceeded" : "timeout exceeded";
          const transitional2 = config.transitional || transitionalDefaults2;
          if (config.timeoutErrorMessage) {
            timeoutErrorMessage = config.timeoutErrorMessage;
          }
          abort(new AxiosError2(
      ...
      if (utils$12.isStream(data)) {
        ...
        data.on("close", () => {
          if (!ended && !errored) {
            abort(new CanceledError2("Request stream has been aborted", config, req));
          }
        });
        data.pipe(req);
      } else {
  ...
  const isURLSameOrigin2 = platform2.hasStandardBrowserEnv ? /* @__PURE__ */ ((origin3, isMSIE) => (url2) => {
  ...
  const cookies2 = platform2.hasStandardBrowserEnv ? (
    // Standard browser envs support document.cookie
    {
      write(name, value, expires, path2, domain, secure, sameSite) {
        if (typeof document === "undefined") return;
  ...
  const xhrAdapter2 = isXHRAdapterSupported2 && function(config) {
    return new Promise(function dispatchXhrRequest(resolve, reject) {
      ...
      request.onerror = function handleError(event) {
        const msg = event && event.message ? event.message : "Network Error";
        const err = new AxiosError2(msg, AxiosError2.ERR_NETWORK, config, request);
        err.event = event || null;
        reject(err);
        request = null;
      };
      ...
      if (_config.cancelToken || _config.signal) {
        onCanceled = (cancel) => {
          ...
          reject(!cancel || cancel.type ? new CanceledError2(null, config, request) : cancel);
          request.abort();
          request = null;
        };
        _config.cancelToken && _config.cancelToken.subscribe(onCanceled);
  ...
  const composeSignals$1 = composeSignals2;
  ...
  const globalFetchAPI2 = (({ Request, Response }) => ({
    Request,
    Response
  }))(utils$12.global);
  const {
    ReadableStream: ReadableStream$12,
    TextEncoder: TextEncoder$12
  } = utils$12.global;
  const test2 = (fn, ...args) => {
    try {
      return !!fn(...args);
  ...
  const factory2 = (env) => {
    ...
    const encodeText = isFetchSupported && (typeof TextEncoder$12 === "function" ? /* @__PURE__ */ ((encoder) => (str) => encoder.encode(str))(new TextEncoder$12()) : async (str) => new Uint8Array(await new Request(str).arrayBuffer()));
    ...
    isFetchSupported && (() => {
      ["text", "arrayBuffer", "blob", "formData", "stream"].forEach((type2) => {
        !resolvers[type2] && (resolvers[type2] = (res, config) => {
          ...
          throw new AxiosError2(`Response type '${type2}' is not supported`, AxiosError2.ERR_NOT_SUPPORT, config);
        });
      });
    })();
    const getBodyLength = async (body) => {
    ...
    return async (config) => {
      ...
      try {
        ...
        if (err && err.name === "TypeError" && /Load failed|fetch/i.test(err.message)) {
          throw Object.assign(
            new AxiosError2("Network Error", AxiosError2.ERR_NETWORK, config, request),
            {
              cause: err.cause || err
            }
          );
        }
        throw AxiosError2.from(err, err && err.code, config, request);
      }
    };
  };
  ...
  function getAdapter2(adapters3, config) {
    ...
    for (let i = 0; i < length; i++) {
      ...
      if (!isResolvedHandle2(nameOrAdapter)) {
        ...
        if (adapter === void 0) {
          throw new AxiosError2(`Unknown adapter '${id}'`);
        }
      }
  ...
  const validators$12 = {};
  ["object", "boolean", "number", "function", "string", "symbol"].forEach((type2, i) => {
    validators$12[type2] = function validator3(thing) {
      return typeof thing === type2 || "a" + (i < 1 ? "n " : " ") + type2;
    };
  });
  ...
  class CancelToken3 {
    constructor(executor) {
      ...
      executor(function cancel(message, config, request) {
        ...
        token.reason = new CanceledError2(message, config, request);
        resolvePromise(token.reason);
      });
    }
    /**
  ...
  const CancelToken$12 = CancelToken3;
  ...
  const HttpStatusCode$12 = HttpStatusCode2;
  function createInstance2(defaultConfig) {
    const context = new Axios$12(defaultConfig);
    const instance = bind2(Axios$12.prototype.request, context);
    utils$12.extend(instance, Axios$12.prototype, context, { allOwnKeys: true });
...
function loadEnvValue(filePath) {
  if (!fs__namespace.existsSync(filePath)) return void 0;
  const result = mainExports.config({ path: filePath });
...
function getBackendUrl() {
  ...
  const candidates = [
    path__namespace.resolve(process.cwd(), ".env"),
    path__namespace.resolve(process.cwd(), ".env.production")
  ];
  if (process.resourcesPath) {
    candidates.push(path__namespace.join(process.resourcesPath, ".env"));
    candidates.push(path__namespace.join(process.resourcesPath, ".env.production"));
  }
  ...
  if (!backendUrl && electron.app?.isPackaged) {
...
function registerInvoiceHandlers() {
  ...
  electron.ipcMain.handle("save-invoice", async (_event, invoiceData) => {
    try {
      ...
      const { InvoiceDataSchema, validateData } = await Promise.resolve().then(() => require("./validation.schema-DSaoo8h-.js"));
  ...
  electron.ipcMain.handle("get-next-invoice-number", async (_event, patientData) => {
    try {
...
function registerPatientHandlers() {
  ...
  electron.ipcMain.handle("delete-patient", async (_event, patientId, target) => {
    try {
...
function registerPresetHandlers() {
  ...
  electron.ipcMain.handle("sync-presets-from-cloud", async () => {
    try {
      ...
      console.log(`📋 Syncing presets from: ${backendUrl2}/api/presets`);
      const response = await axios.get(`${backendUrl2}/api/presets`, {
...
function getSettings() {
  ...
  return {
    invoiceSaveLocation: path__namespace.join(electron.app.getPath("downloads"), "Invoices")
  };
}
...
function registerSettingsHandlers() {
  electron.ipcMain.handle("get-save-location", async () => {
    const settings = getSettings();
    return { success: true, location: settings.invoiceSaveLocation };
  });
  electron.ipcMain.handle("set-save-location", async (_event, location) => {
    try {
      const settings = getSettings();
  ...
  electron.ipcMain.handle("choose-save-location", async () => {
    try {
...
function registerPrintHandlers() {
  electron.ipcMain.handle("print-invoice", async (_event, htmlContent, invoiceData) => {
    try {
  ...
  electron.ipcMain.handle("print-invoice-and-preview", async (_event, htmlContent, invoiceData) => {
    try {
      const printWindow = new electron.BrowserWindow({
        show: false,
        width: 1200,
  ...
  electron.ipcMain.handle("print-invoice-save-and-preview", async (_event, htmlContent, invoiceData) => {
    try {
      const printWindow = new electron.BrowserWindow({
        show: false,
        width: 1200,
...
function registerSyncHandlers(syncEngine2) {
  const prisma = getPrismaClient();
  electron.ipcMain.handle("sync-now", async () => {
    try {
      ...
      const result = await syncEngine2.performSync(true);
      if (result.success) {
        syncEngine2.resetAutoSyncTimer(5 * 60 * 1e3);
      }
  ...
  electron.ipcMain.handle("force-sync-push", async () => {
    try {
      if (!prisma) throw new Error("Prisma not initialized");
      ...
      if (syncEngine2) {
        return syncEngine2.performSync(true);
      }
  ...
  electron.ipcMain.handle("get-sync-status", async () => {
    try {
      ...
      const status = await syncEngine2.getSyncStatus();
      return { success: true, status };
    } catch (error) {
      return { success: false, error: String(error) };
    }
  });
  electron.ipcMain.handle("reset-local-database", async () => {
    try {
      if (!prisma) {
        throw new Error("Prisma not initialized");
      }
  ...
  electron.ipcMain.handle("reset-sync-timestamp", async () => {
    try {
      if (!syncEngine2) {
        throw new Error("Sync engine not initialized");
      }
      console.log("🔄 Resetting sync timestamp to force full sync...");
      await syncEngine2.resetSyncTimestamp();
  ...
  electron.ipcMain.handle("reset-cloud-database", async () => {
    try {
      const backendUrl = getBackendUrl();
  ...
  electron.ipcMain.handle("reset-all-databases", async () => {
    try {
      ...
      if (syncEngine2) {
        ...
        syncEngine2.startAutoSync(30 * 60 * 1e3);
      }
  ...
  electron.ipcMain.handle("get-database-stats", async () => {
    try {
      if (!prisma) {
        throw new Error("Prisma not initialized");
      }
...
function registerIpcHandlers(syncEngine2) {
  ...
  registerSyncHandlers(syncEngine2);
}
...
if (process.platform === "win32") {
  ...
  if (squirrelCommand && squirrelCommand.startsWith("--squirrel")) {
    ...
    const appFolder = path2.resolve(process.execPath, "..");
    const rootFolder = path2.resolve(appFolder, "..");
    const updateExe = path2.resolve(path2.join(rootFolder, "Update.exe"));
    const exeName = path2.basename(process.execPath);
...
function createWindow() {
  ...
  mainWindow = new electron.BrowserWindow({
    ...
    webPreferences: {
      ...
      contextIsolation: false,
      webSecurity: false
      // Allow ES modules from file:// protocol
    },
  ...
  let viteName = "main_window";
  {
    const indexPath = path__namespace.join(__dirname, `../renderer/${viteName}/index.html`);
...
electron.app.whenReady().then(async () => {
  ...
  try {
    ...
    electron.dialog.showErrorBox("Database Error", `Failed to initialize database: ${error.message}
${error.stack || ""}`);
  }
  let prisma = null;
  try {
    ...
    electron.dialog.showErrorBox("Database Error", `Failed to initialize Prisma: ${error.message}
${error.stack || ""}`);
  }
  const backendUrl = getBackendUrl();
