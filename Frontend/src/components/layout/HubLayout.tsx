...
/**
 * Map a route path to its lazy chunk so we can preload the JS on hover/idle.
 * Keys mirror the routes defined in App.tsx.
 */
const ROUTE_CHUNK_LOADERS: Record<string, () => Promise<unknown>> = {
  '/invoice-generator': () => import('@/pages/InvoiceGenerator'),
  '/invoice-customizer': () => import('@/pages/InvoiceCustomizer'),
  '/finances': () => import('@/pages/Finances'),
  '/reports': () => import('@/pages/Reports'),
  '/inventory': () => import('@/pages/Inventory'),
  '/settings': () => import('@/pages/Settings'),
  '/treatment-settings': () => import('@/pages/TreatmentSettings'),
  '/database-find': () => import('@/pages/DatabaseFind'),
};

const preloaders: Record<string, () => void> = Object.fromEntries(
  Object.entries(ROUTE_CHUNK_LOADERS).map(([path, loader]) => [
    path,
    () => {
      // Fire-and-forget; the resolved promise is discarded because the
      // module system caches the chunk for the actual navigation.
      void loader().catch(() => {});
    },
  ])
);
          ...
          <div className="bg-slate-100/50 p-1 rounded-[20px] flex items-center gap-1 shadow-inner ring-1 ring-slate-200/50 backdrop-blur-md">
            ...
            {tabs.map((tab) => (
              <NavLink
                key={tab.path}
                to={tab.path}
                onMouseEnter={() => preloaders[tab.path]?.()}
                onFocus={() => preloaders[tab.path]?.()}
                className={({ isActive }) =>
