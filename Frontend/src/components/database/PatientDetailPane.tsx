...
export const PatientDetailPane = ({ invoices, onPrintInvoice }: PatientDetailPaneProps) => {
  ...
  if (!invoices || invoices.length === 0) {
      return (
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 h-full flex flex-col items-center justify-center p-12 text-center text-slate-400">
              <svg className="w-16 h-16 mb-4 text-slate-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>
              <h3 className="text-xl font-medium text-slate-500 mb-2">No Patient Selected</h3>
              <p className="text-sm">Select a patient from the list on the left to view their complete details and invoice history.</p>
          </div>
      );
  }
  ...
  return (
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 flex flex-col h-full overflow-hidden">
        {/* Header */}
        <div className="bg-white border-b border-slate-100 px-6 py-6 flex justify-between items-start shrink-0 z-10">
                ...
                {showDeleteOptions && (
                    <div className="absolute right-0 top-full mt-2 w-48 bg-white border border-slate-200 rounded-xl shadow-xl z-50 p-1 flex flex-col gap-1">
                        ...
                        <button 
                            ...
                            className={`text-left px-3 py-2 text-sm rounded-lg transition-colors flex items-center gap-2 ${!hasCloudData ? 'text-slate-400 opacity-50 cursor-not-allowed bg-slate-50' : 'text-slate-700 hover:bg-rose-50 hover:text-rose-600'}`}
                        ...
                        <button 
                            ...
                            className={`text-left px-3 py-2 text-sm font-semibold rounded-lg transition-colors flex items-center gap-2 ${!hasCloudData ? 'text-rose-300 opacity-50 cursor-not-allowed bg-slate-50' : 'text-rose-600 hover:bg-rose-50'}`}
                            title={!hasCloudData ? "Not synced to Cloud" : ""}
                        >
